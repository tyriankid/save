...
static public class NGUITools
{
	...
	static public AudioSource PlaySound (AudioClip clip, bool loop = false) { return PlaySound(clip, 1f, 1f, loop); }
	...
	static public AudioSource PlaySound (AudioClip clip, float volume, float pitch, bool loop = false)
	{
		volume *= soundVolume;

        if (clip != null && (volume > 0.01f || loop))
		{
			if (mListener == null)
			{
			...
			{
                GameObject obj = new GameObject("__AUDIO_SOURCE_" + clip.name);
